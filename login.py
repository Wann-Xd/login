...
def manual():
	try:
		ses = requests.Session()
		print('\nContoh : c.txt & t.txt')
		masuk = input('\nMasukan Nama File Cokie : ')
		masok = input('Masukan Nama File Token : ')
		cookie = input('\nMasukan Cookie : ')
		cookies = {'cookie':cookie}
		url = 'https://www.facebook.com/adsmanager/manage/campaigns'
		req = ses.get(url,cookies=cookies)
		...
		roq = ses.get(nek,cookies=cookies)
		tok = re.search('accessToken="(.*?)"',str(roq.content)).group(1)
		token = open(masok,'a').write(tok)
		cokie = open(masuk,'a').write(cookie)
		print(f'\nLogin Berhasil , file tersimpan di {masok} & {masuk}')
	except Exception as e:
		os.system(f'rm -rf {masuk} && rm -rf {masok}')
		print(e)
		exit()
